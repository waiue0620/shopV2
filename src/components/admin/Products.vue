...
export default {
  ...
  methods: {
    ...
    openModal (isNew, item) {
      const vm = this
      vm.isNew = isNew
      document.getElementById('customFile').setAttribute('type', 'text')
      document.getElementById('customFile').setAttribute('type', 'file')
      if (isNew) {
        vm.tempProduct = {}
      } else {
    ...
    updataProduct () {
      ...
      vm.$store.dispatch('adminModules/updataProduct', { httpMethod: httpMethod, tempProduct: vm.tempProduct })
    },
    openDelModal (item) {
      const vm = this
    ...
    delProduct () {
      const vm = this
      vm.$store.dispatch('adminModules/delProduct', vm.tempProduct)
    },
    uploadFile () {
      console.log(this)
      const uploadedFile = this.$refs.files.files[0]
      const vm = this
      this.$store.dispatch('adminModules/uploadFile', uploadedFile).then(() => {
        // console.log(vm.imageUrl)
        vm.$set(vm.tempProduct, 'imageUrl', vm.imageUrl)
      })
    }
  },
  created () {
  ...
  computed: {
    ...mapGetters(['status']),
    ...mapGetters('adminModules', ['products', 'pagination', 'imageUrl'])
  }
}
</script>
